...
const Advanced = () => {
    ...
    const [imputationDetails, setImputationDetails] = useState([]);
    const [isImputationDetailsCollapsed, setIsImputationDetailsCollapsed] = useState(true);
   ...
   useEffect(() => {
    ...
    const handleMissingValues = (data) => {
        ...
        try {
                           ...
                           updatedData = updatedData.map((row, rowIndex) => {
                               if (row[col] === null || row[col] === undefined) {
                                   ...
                                   if (featureRow.length > 0) {
                                       imputedValue = regressionModel.predict(featureRow[0]);
                                   } else {
                                       const validValues = data.filter(row => row[col] !== null && row[col] !== undefined).map(row => row[col]);
                                       imputedValue = validValues.length > 0 ? ss.mean(validValues) : null;
                                   }
                                   
                                   imputationDetails.push({
                                       row: rowIndex,
                                       column: col,
                   ...
                   updatedData = updatedData.map((row, rowIndex) => {
                       if (row[col] === null || row[col] === undefined) {
                           ...
                           imputationDetails.push({
                               ...
                               column: col,
                               originalValue: originalValue,
                               imputedValue: mostFrequentValue,
            ...
            return { data: updatedData, message: `Imputed ${missingValueCount} missing values.`, imputationDetails: imputationDetails };
        } catch (error) {
            return { data, message: `Error handling missing values: ${error}`, imputationDetails: [] };
        }
    };
  ...
  const handleOutliers = (data, numericColumns, threshold, originalData, handleOutliersFlag, labelColumn) => {
    ...
    const applyDataTuning = async () => {
        ...
        const { data: processedData, messages, imputationDetails } = preprocessDataWithMessages(tunedData, handleCommasAsDecimalFlag);
        setImputationDetails(imputationDetails); 
        ...
        if (file && file.name.endsWith('.csv')) {
            ...
            Papa.parse(file, {
                complete: (result) => {
                    ...
                    setHeatmapData(null);
                    const { data: cleanedData, messages, imputationDetails } = preprocessDataWithMessages(rawData, true);
    ...
    const preprocessDataWithMessages = (data, treatCommasAsDecimal = false) => {
        if (!data || data.length === 0) return {data, messages: {}, imputationDetails: []};
       ...
       return {
          ...
          messages: {
           ...
           imputationDetails: missingResult.imputationDetails 
        };
    };
  ...
  const selectColumns = (data) => {
    ...
    if (numericColumns.length === 0 && !handleCategoricalDataFlag) {
      ...
      return (
              ...
              <div className='import-controls'>
                  ...
                  <div className={`toggle-options-container ${optionsOpen ? '' : 'collapsed'}`}>
                      ...
                      <div className="toggle-option"
                           ...
                           onMouseLeave={handleTooltipLeave}
                      >
                          <input type="checkbox" id="handleCommas" checked={handleCommasAsDecimalFlag} onChange={() =>  {
                              const newValue = !handleCommasAsDecimalFlag; 
                              setHandleCommasAsDecimalFlag(newValue); 
                      ...
                      <div className="toggle-option"
                           ...
                           onMouseLeave={handleTooltipLeave}
                      >
                          <input type="checkbox" id="handleCategorical" checked={handleCategoricalDataFlag} onChange={() => {
                              const newValue = !handleCategoricalDataFlag; 
                              setHandleCategoricalDataFlag(newValue)
                              window.location.reload(); 
                          }} />
                      ...
                      <div className="toggle-option"
                           ...
                           onMouseLeave={handleTooltipLeave}
                      >
                          <input type="checkbox" id="handleMissing" checked={handleMissingValuesFlag} onChange={() => {
                              const newValue = !handleMissingValuesFlag;
                              setHandleMissingValuesFlag(!handleMissingValuesFlag)
                              window.location.reload(); 
                          }} />
        ...
        {imputationDetails.length > 0 && (
        <div className="imputation-details">
            <h2 className="status-message-header">Imputation Details</h2>
            <button onClick={() => setIsImputationDetailsCollapsed(!isImputationDetailsCollapsed)}>
                {isImputationDetailsCollapsed ? 'Show Imputation Details' : 'Hide Imputation Details'}
            </button>

            {!isImputationDetailsCollapsed && (
                <table className="data-table">
                    <thead>
                        <tr>
                            <th>Row</th>
                    ...
                    <tbody>
                        {imputationDetails.map((detail, index) => (
                            ...
                            <td>{detail.row}</td>
                            <td>{detail.column}</td>
                            <td>{detail.originalValue === null || detail.originalValue === undefined ? 'NULL' : String(detail.originalValue)}</td>
                            <td>{detail.imputedValue !== null && detail.imputedValue !== undefined ? detail.imputedValue.toFixed(2) : 'N/A'}</td>
                            <td>{detail.method}</td>
                            </tr>
                        ))}
                    </tbody>
